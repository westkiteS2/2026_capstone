  ...
  function trackLoginAttempt(e) {
    ...
    if (form) {
      ...
      for (let i = pwInputs.length - 1; i >= 0; i--) {
        // [중요] 네이버가 폼 값을 오염시켰더라도, 안전한 백업(passwordBackups)에서 값을 꺼내옵니다.
    ...
    window.trackTimer = setTimeout(() => {
      window.PwUtils?.sha1Hex(targetPassword).then((hash) => {
        console.log('[PwGuard] 🔒 생성된 해시값:', hash)

        // [추가된 기능] OTP 입력창 존재 여부에 따른 분기 처리
        if (hasOtpInput()) {
          if (
            confirm(
              `[PwGuard] OTP 인증이 감지되었습니다.\n${domain} (${username}) 계정의 비밀번호를 안전하게 보관하시겠습니까?`,
            )
          ) {
            chrome.runtime.sendMessage(
              {
                action: 'directRecordHash', // 백엔드 다이렉트 저장 액션
                payload: { hash, domain, username },
              },
              (response) => {
                if (response && response.success) {
                  alert('성공적으로 보관되었습니다. 남은 인증을 진행해 주세요.')
                }
              },
            )
          }
